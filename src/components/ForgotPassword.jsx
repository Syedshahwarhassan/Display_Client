...
//   const navigate = useNavigate()
  ...
  const handlelogin = async (e)=>{
    e.preventDefault()
  try {
    const {data} = await axios.post("http://localhost:4600/u1/forgot", {email})
    if(data.success === true){
     toast.success("check link our your email")
    }
  ...
  return (
    <div>
        <h2 className="text-center text-4xl font-bold mt-10">Forgot Password</h2>
     
        <form className="space-y-6 font-[sans-serif] my-10 mb-40 border p-4 max-w-lg mx-auto" onSubmit={handlelogin}>
        <div className="relative flex items-center">
          <input type="email" placeholder="Enter Email" name="email" onChange={(e)=>setEmail(e.target.value)} className="px-2 py-3 bg-white text-black w-full text-sm border-b-2 focus:border-blue-500 outline-none" />
